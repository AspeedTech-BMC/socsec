...
def insert_manifest_vendor_sigs(input_manifest_file, ecc_sig_file, lms_sig_file, output_manifest_file):
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Insert manifest vendor ECC & LMS keys and signatures into a SoC manifest file."
    )
    parser.add_argument("input_manifest_file", help="Path to the input SoC manifest file.")
    parser.add_argument("ecc_sig_file", help="Path to the ECC signature file (96 bytes).")
    parser.add_argument("lms_sig_file", help="Path to the LMS signature file (1620 bytes).")
    parser.add_argument("output_manifest_file", help="Path to write the updated SoC manifest file.")

    args = parser.parse_args()

    insert_manifest_vendor_sigs(
