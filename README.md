...
```bash
socsec verify --sec_image /path/to/s_u-boot-spl.bin --otp_image /path/to/otp-all.image
```
...
This tool is used to generate OTP image.

### Platform Support
- AST2600, AST10x0, AST27xx
...
### Export OTP image
```bash
usage: otptool export [-h] [--soc SOC] --output OUTPUT [--excel EXCEL] otp_image

positional arguments:
  otp_image      OTP image. Can be either an `otp-ast2700.bin` (raw device dump), `otp-all.bin` (generated by `otptool`), `otp-all.image` (generated by `otptool` and including a header).

optional arguments:
  -h, --help     show this help message and exit
  --soc SOC      soc id (e.g. 2700)
  --output OUTPUT
                 output jsonc file
  --excel EXCEL  OTP Excel map file to extract field metadata (e.g. AST2700A2_OTP_memory_map.xlsx)
```

The `export` command is an addition to `otptool` that allows reverse-parsing compiled OTP binaries back into human-readable configuration formats. It reconstructs a detailed JSON object representing the exact state of all OTP registers by using an Excel Map (Memory Map) spreadsheet as a metadata dictionary. The resulting JSON includes the field names, their exact byte and bit addresses, descriptions from the Excel file, and accurately scaled sizes.

**Export Features:**
- Works gracefully against both natively padded `otptool` `.image` objects and standalone hardware `.bin` files automatically.
- Correctly accounts for duplicated configurable logic (e.g. `Duplicate of 422`) by appending base offsets algorithmically to avoid JSON dictionary collision.
- Automatically handles intelligent `Little-Endian` byte conversion of fields to format their physical states onto readable JSON values.
- Formats excessively long hex keys seamlessly into arrays of `< 64 byte` chunked strings to avoid unreadable file wrap.

## Supplemental Tools (Caliptra Workflow)

The [tools/](tools/) directory contains utility scripts mapping directly to the most critical firmware and root of trust requirements on the AST2700 platform. The typical Caliptra workflow consists of the following 5 main steps:
...
### 2. Program Key Hash into OTP
Once the `owner_key_hash.bin` is generated, assign its path inside your OTP JSON configuration file (e.g. `2700-a2_sample-full.jsonc`) under the `caliptra_region`. Then generate the OTP image to be programmed into the system:
```bash
otptool make_otp_image --key_folder <key_folder> 2700-a2_sample-full.jsonc
```
*Output: `otp-all.image` (Use the AST2700 u-boot/linux OTP utility to program this file)*
